...
class GOPRO(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg):
        ...
        for video_name in self.video_list:
            ...
            frames_in_video = [
                os.path.join(video_name, frame) for frame in frames_in_video
            ]
            ...
            if cfg.sampling == "n_n" or cfg.sampling == "n_l":
                ...
                if cfg.overlapping:
                    ...
                    self.frames += frames_in_video[
                        : len(frames_in_video) - sampled_frames_length + 1
                    ]
                else:
                    # ensure the sampling frame can be sampled!
                    self.frames += frames_in_video[
                        : len(frames_in_video)
                        - sampled_frames_length
                ...
                if cfg.overlapping:
                    self.frames += frames_in_video[
                        sampled_frames_length // 2 : len(frames_in_video)
                        - (sampled_frames_length // 2)
                    ]
                else:
                    self.frames += frames_in_video[
                        sampled_frames_length // 2 : len(frames_in_video)
                        - (sampled_frames_length // 2) : sampled_frames_length
                    ]
                ...
                if cfg.overlapping:
                    ...
                    self.frames += frames_in_video[
                        sampled_frames_length - 1 :: sampled_frames_length
                    ]
        ...
        logger.info(f"Total samples {len(self.frames)} are loaded for {self.cfg.mode}!")
    ...
    def __getitem__(self, idx):
        ...
        if self.cfg.sampling == "n_c":
            input_frames_name = [
                "{:06d}.{}".format(i, suffix)
                for i in range(
                    frame_idx - (self.cfg.num_frames // 2) * self.cfg.interval,
                    frame_idx + (self.cfg.num_frames // 2) * self.cfg.interval + 1,
                    self.cfg.interval,
                )
            ]
            ...
            input_frames_name = [
                ...
                for i in range(
                    frame_idx,
                    frame_idx + self.cfg.interval * self.cfg.num_frames,
                    self.cfg.interval,
                )
            ]
            ...
            input_frames_name = [
                "{:06d}.{}".format(i, suffix)
                for i in range(
                    frame_idx - self.cfg.num_frames * self.cfg.interval + 1,
                    frame_idx + 1,
                    self.cfg.interval,
                )
            ]
        ...
        assert (
            len(input_frames_name) == self.cfg.num_frames
        ), "Wrong frames length not equal the sampling frames {}".format(
            self.cfg.num_frames
        )
        ...
        gt_frames = [
            read_img_opencv(gt_frames_path.format(frame_name))[..., ::-1]
            for frame_name in gt_frames_name
        ]
        input_frames = [
            read_img_opencv(input_frames_path.format(frame_name))[..., ::-1]
...
@DATASET_REGISTRY.register()
class GOPRO_rearrange(torch.utils.data.Dataset):  # zzh
    """
    GOPRO dataset with a rearranged dir structure ('gt/GOPR0384_11_00' instead or original 'GOPR0384_11_00/sharp').
    Args:
        cfg(Easydict): The config file for dataset.
            root_gt(str): the root path of gt videos
            root_input(str): the root path of the input videos
    ...
    def __init__(self, cfg):
        ...
        for video_name in self.video_list:
            # Warning! change the video path in different format of video deblurring dataset
            # video_path = os.path.join(self.cfg.root_gt, video_name, "sharp")
            video_path = os.path.join(self.cfg.root_gt, video_name)  # gt vid path
            frames_in_video = os.listdir(video_path)
            frames_in_video.sort()
